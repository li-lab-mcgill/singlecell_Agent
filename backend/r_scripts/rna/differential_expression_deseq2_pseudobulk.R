...
read_h5ad_components <- function(path) {
  ...
  if (!requireNamespace("reticulate", quietly = TRUE)) {
    stop("DESeq2 pseudobulk requires zellkonverter or reticulate to read .h5ad files.")
  }
...
for (target in targets) {
  ...
  meta <- pb$meta
  pb_counts <- round(as.matrix(pb$counts))
  ...
  design_formula <- if (use_paired) ~ sample_id + condition else ~ condition
  dds <- DESeq2::DESeqDataSetFromMatrix(countData = pb_counts, colData = meta, design = design_formula)
  min_samples <- min(table(meta$condition))
  keep_genes <- rowSums(DESeq2::counts(dds) >= 10) >= min_samples
  dds <- dds[keep_genes, ]
  if (nrow(dds) == 0) {
    skipped[[target]] <- "all genes filtered after low-count removal"
    next
  }
  ...
  res_df$group <- target
  res_df$reference <- if (identical(reference, "rest")) "rest" else reference
  res_df$method <- "deseq2_pseudobulk"
  res_df$n_target_samples <- sum(meta$condition == "target")
  res_df$n_reference_samples <- sum(meta$condition == "reference")
  res_df$paired_design <- use_paired
  res_df <- res_df[order(res_df$padj, res_df$pvalue, decreasing = FALSE, na.last = TRUE), , drop = FALSE]
  res_df$rank <- seq_len(nrow(res_df))
  keep_names <- c("group", "reference", "rank", "gene", "log2FoldChange", "pvalue", "padj", "stat", "baseMean", "method", "n_target_samples", "n_reference_samples", "paired_design")
  results[[target]] <- res_df[, keep_names]
  names(results[[target]]) <- c("group", "reference", "rank", "gene", "lfc", "pval", "pval_adj", "statistic", "base_mean", "method", "n_target_samples", "n_reference_samples", "paired_design")
}
