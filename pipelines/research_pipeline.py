...
def main() -> None:
    cur_path = str(Path(__file__).resolve().parents[1])

    parser = argparse.ArgumentParser()
    parser.add_argument("--code-dir", default="saved_code")
    parser.add_argument("--results-dir", default="results")
    parser.add_argument("--tool-artifact-dir", default=None, help="Directory for intermediate artifacts produced by tool/coder/optimizer calls.")
    ...
    parser.add_argument("--background", default=None, help="Optional background/task override for the research pipeline.")
    ...
    parser.add_argument("--consultant-tool-only", action=argparse.BooleanOptionalAction, default=False)
    args = parser.parse_args()

    file_path = args.input_mod1 or f"{cur_path}/data/pbmc_RNA_count.annotated.h5ad"
    if not Path(file_path).exists():
        raise FileNotFoundError(
            f"input_mod1 not found: {file_path}. Pass --input_mod1 with an existing .h5ad path."
        )
    mod2_path = args.input_mod2

    global tg
    global AnalystAgent, ConsultantAgent, Config
    global build_consultant_history_context, plan_fingerprint, record_consultant_plan
    global EVALUATOR_MEETING_ORDER, critic_plan_from_payload, synthesize_critic_payload_from_evaluators
    global TextGradEvaluator, build_instruction_text, build_open_issues, parse_critic_output
    global parse_evaluator_message, to_float, update_primary_metric_state
    global CodeExecutor, StageScriptGenerator
    global ScriptNotebook, append_decision_record, append_history_note, append_script_note_record
    global build_code_diff, build_current_diffs_payload, build_history_digest, build_missing_code_note
    global build_script_notes_history, infer_design_identity, fetch_mcp_tools_text
    global ConsultantPlanRecord, DecisionLedgerRecord, GlobalBestState, HistoryNoteRecord, OuterLoopState
    global ScriptNoteRecord, STAGE_FILENAMES, PaperBackend, ConsultantRAGAgent, BackendConfig
    global SingleCellBackend, write_failure_record
    ...
    from pipelines.hist_notebook import (
        ScriptNotebook,
    ...
    from pipelines.mcp_utils import fetch_mcp_tools_text
    from pipelines.multieval_types import (
    ...
    from pipelines.validator import write_failure_record
#     background = f"""
...
# """
    background = str(args.background or "").strip() or f"""
Data file: Modality 1: {file_path}
DATA: Sparse, high-dimensional gene expression counts.
Each row represents a cell and each column represents a gene.
...
"""
    ...
    single_cell_backend = SingleCellBackend(
        BackendConfig(
            cache_dir=Path(config.result_dir) / "backend_cache",
            scratch_dir=Path(config.result_dir) / "backend_scratch",
            log_dir=Path(config.result_dir) / "backend_logs",
            reticulate_python=Path(sys.executable),
        )
    )
    single_cell_backend.bind_runtime_context(config)

    if args.consultant_tool_only:
        tool_artifact_dir = Path(args.tool_artifact_dir) if args.tool_artifact_dir else Path(config.result_dir) / "tool_artifacts"
        dispatcher = SessionDispatcher(
            router=None,
            tool_consultant=ToolConsultantAgent(
                engine_name=args.engine,
                result_dir=config.result_dir,
                backend=single_cell_backend,
            ),
            ...
            research_executor=SubprocessResearchExecutor(
                repo_root=cur_path,
                engine_name=args.engine,
                input_mod1=file_path,
                input_mod2=mod2_path,
                dataset_dir=config.dataset_dir,
                time_budget=args.time_budget,
                base_artifact_dir=tool_artifact_dir,
            ),
            tool_artifact_dir=tool_artifact_dir,
            state_store=SessionStateStore(Path(config.result_dir) / "session_state.json"),
            result_dir=config.result_dir,
        ...
        tool_only_result = dispatcher.execute_task(
            user_message=background.strip(),
            session_state={
                "input_h5ad_path": file_path,
                "input_mod2_path": mod2_path,
                "tool_artifact_dir": str(tool_artifact_dir),
                "background": background.strip(),
            },
            session_tag="consultant_tool_only",
        )
        print(f"consultant_tool_only_output:\n{json.dumps(tool_only_result, indent=2, ensure_ascii=False)}")
        return
    ...
    paper_backend = PaperBackend(rag_agent=rag_agent, config=config, prepared_contexts=prepared_rag_contexts)
    ...
    analyst = AnalystAgent(
        ...
        result_dir=config.result_dir,
        single_cell_backend=single_cell_backend,
    )
